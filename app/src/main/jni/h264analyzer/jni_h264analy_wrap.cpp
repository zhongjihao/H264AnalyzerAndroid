...
static struct InfoCustomersNativeJNI{
    jclass     naluitem_cls;
...
static void initnaluItemNativeJNI(JNIEnv* env,const char* className)
{
    ...
    //获取JAVA类NaluItem中setNal_index方法ID
    if(g_naluItemNativeJNI.setNal_indexID == NULL){
        g_naluItemNativeJNI.setNal_indexID = env->GetMethodID(g_naluItemNativeJNI.naluitem_cls,"setNal_index","(I)V");
        if(g_naluItemNativeJNI.setNal_indexID == NULL){
            LOGE("=======not get c++ class NaluItem setNal_indexID======");
            return;
        }
    }
...
jobjectArray JNI_h264_nal_parse(JNIEnv *env,jclass jcls __unused,jlong jcPtr,jstring jfilepath)
{
    ...
    std::vector<NALU_ITEM*>& naluSets = pNalParse->h264_nal_parse(filePath);
    int elementCount = naluSets.size();
    initnaluItemNativeJNI(env,"com/android/h264/h264analyzerandroid/NaluItem");
    LOGD("%s: ===zhongjihao===elementCount: %d",__FUNCTION__,elementCount);
    ...
    return resultArray;//从该函数返回，创建的本地引用resultArray对象数组会被JVM自动回收
}
